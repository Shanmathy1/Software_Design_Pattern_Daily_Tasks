...
//       // Extract token from response (assuming JWT token is returned)
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (email === 'admin@gmail.com' && password === 'admin@2004') {
      ...
      navigate('/admindashboard');
      return;
    } else if (email === 'user@gmail.com' && password === 'user@2006') {
      localStorage.setItem('token', 'fake-user-token');
      navigate('/userdashboard');
      return;
    }

    // If credentials do not match the hardcoded ones, proceed with the API call
    try {
      const response = await axios.post('http://localhost:7777/api/auth/login', {
        email,
        password,
